...
const cleanReport:CleanReport={movedCount:1,movedBytes:200,failedBytes:0,failures:[],freeBefore:500_000,freeAfter:500_000,trashBefore:{bytes:null,error:"Operation not permitted"},trashAfter:{bytes:null,error:"Operation not permitted"},refreshedScan:{...report,scanId:"scan-2",candidates:report.candidates.slice(0,2)}};
...
describe("CleanDisk 四步向导",()=>{
 ...
 it("项目依赖确认后进入独立完成页，废纸篓无权限时不显示为零",async()=>{await finish();const section=screen.getByText("项目依赖",{selector:"h3"}).closest("section")!;fireEvent.click(within(section).getByRole("button",{name:"全选分组"}));fireEvent.click(screen.getByRole("button",{name:"移入废纸篓"}));expect(screen.getByText(/之后需要通过包管理器重新安装/)).toBeInTheDocument();fireEvent.click(screen.getByRole("button",{name:"确认移动"}));await screen.findByText("清理完成",{selector:"h2"});expect(screen.getByText(/macOS 未授权读取废纸篓内容/)).toBeInTheDocument();fireEvent.click(screen.getByRole("button",{name:"查看剩余候选"}));await waitFor(()=>expect(screen.getByText("扫描结果")).toBeInTheDocument());expect(screen.queryByText("node_modules")).not.toBeInTheDocument();});
});
