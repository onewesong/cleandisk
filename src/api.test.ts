...
describe("Tauri command bridge",()=>{
 ...
 it("发送所选类别并转发有序 Channel 事件",async()=>{const events:string[]=[];await beginScan(["application-caches","logs"],event=>events.push(event.event));expect(mocks.invoke).toHaveBeenCalledWith("begin_scan",expect.objectContaining({options:{projectRoots:null,categories:["application-caches","logs"]},onEvent:expect.anything()}));mocks.channels[0].onmessage({event:"plugin_started",plugin:"a",category:"logs",plugin_index:1,total_plugins:1});mocks.channels[0].onmessage({event:"progress",plugin:"a",category:"logs",path:"/a",plugin_index:1,total_plugins:1,visited:1,found:1,bytes:2});mocks.channels[0].onmessage({event:"completed",report:{}});expect(events).toEqual(["plugin_started","progress","completed"]);});
