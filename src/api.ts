...
import type { CleanEvent, CleanReport, ScanEvent, Settings } from "./types";
export const getSettings=()=>invoke<Settings>("get_settings");
export const saveSettings=(projectRoots:string[])=>invoke<Settings>("save_settings",{input:{projectRoots}});
export function beginScan(onEvent:(event:ScanEvent)=>void){const channel=new Channel<ScanEvent>();channel.onmessage=event=>{onEvent(event);if(["completed","cancelled","failed"].includes(event.event))channel.onmessage=()=>{}};return invoke<string>("begin_scan",{options:{projectRoots:null},onEvent:channel});}
